...
export function factory(templateService: TemplateService) {
  return new WizardScene(
    ...
    async (ctx: any) => {
      ...
      const message =
        'Please choose a template to share. You can use /cancel to abort, or /new to create a new template.'
      return select.promptForOption(message, templateOptions, ctx)
    },
    select.extractSelection(
      'template',
      ({ value: template }) =>
        `OK, you selected ${template.name}. Now, please, forward me a message from the person you want to share the template with`
    ),
    async (ctx: any) => {
      ...
      if (newOwner.is_bot)
        return ctx.reply(
          "I'm sorry, but you can't share a template with a chatbot. Please, choose a regular user"
        )
